...
import resolveWP from "./resolve-wp";
import {
    Plugin,
    loadEnv,
    UserConfig,
...
interface gutenbergPlugin extends Plugin {
 ...
 */
function resolvegutenbergPlugin(
    ...
    return {
        name: "gutenberg",
        enforce: "post",
        options: resolveWP.options,
        outputOptions: resolveWP.outputOptions,
        ...
        transform(code) {
            if (resolvedConfig.command === "serve") {
                return code.replace(
                    /__gutenberg_vite_placeholder__/g,
                    viteDevServerUrl
                );
            }
        },
...
function resolvePluginConfig(
    ...
    if (typeof config.input === "undefined") {
        throw new Error(
            'gutenberg-vite-plugin: missing configuration for "input".'
        );
    }
    ...
    if (typeof config.publicDirectory === "string") {
        ...
        if (config.publicDirectory === "") {
            throw new Error(
                "gutenberg-vite-plugin: publicDirectory must be a subdirectory. E.g. 'public'."
            );
        }
    }
    ...
    if (typeof config.buildDirectory === "string") {
        ...
        if (config.buildDirectory === "") {
            throw new Error(
                "gutenberg-vite-plugin: buildDirectory must be a subdirectory. E.g. 'build'."
            );
        }
    }
    ...
    return {
        ...
        hotFile:
            config.hotFile ??
            path.join(config.publicDirectory ?? "static", "hot"),
    };
}
